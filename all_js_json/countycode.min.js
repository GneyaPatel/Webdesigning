...
const selectBoxes = document.querySelectorAll('.select-country-code .select-box');

if (selectBoxes.length > 0) {
    selectBoxes.forEach((box) => {
        const optionsContainer = box.querySelector('.options ol');
        ...
        const selectedDiv = box.querySelector('.selected-option div');
        selectedDiv.addEventListener('click', function () {
            box.querySelector('.options').classList.toggle('active');
        ...
        box.querySelectorAll('.option').forEach(option => {
            option.addEventListener('click', function () {
                ...
                const selectedOptionDiv = box.querySelector('.selected-option div');
                selectedOptionDiv.innerHTML = ''; // Clear
                selectedOptionDiv.appendChild(icon);
                selectedOptionDiv.appendChild(phoneCode);

                const hiddenInput = box.querySelector('.selected-option input[type="hidden"][name="country_code"]');
                if (hiddenInput) {
                    hiddenInput.value = phoneCode.textContent;
                }

                box.querySelector('.options').classList.remove('active');
                selectedOptionDiv.classList.remove('active');

                const searchBox = box.querySelector('.search-box');
                if (searchBox) {
                    searchBox.value = '';
                    box.querySelectorAll('.option.hide').forEach(el => el.classList.remove('hide'));
                }
            });
        });

        // Handle live search
        const searchInput = box.querySelector('.search-box');
        if (searchInput) {
            searchInput.addEventListener('input', function () {
                const query = this.value.toLowerCase();
                box.querySelectorAll('.option').forEach(option => {
                    const name = option.querySelector('.country-name').textContent.toLowerCase();
                    option.classList.toggle('hide', !name.includes(query));
                });
            });
        }
    });
...
if(select_box != null){
	$('.select-country-code .select-box').each(function(){
		let options = null;
		main = $(this);
		...
		$.each(countries, function(index, country) {
			...
			// Append the option to the <ol> element inside select_box
			main.find('.options').find('ol').append(option);
			// console.log($(this).find('.options'));
		});
		
	});
	$('.select-country-code .select-box .selected-option div').on('click',function(){
		main = $(this).parents('.select-box');
		main.find('.options').toggleClass('active');
		main.find('.selected-option div').toggleClass('active');
	});

	$('.select-country-code .select-box .option').on('click',function(){
		main = $(this).parents('.select-box');

		const icon = $(this).find('.iconify').clone(true),
		phone_code = $(this).find('strong').clone(true);
		
		// console.log(phone_code.text());
			
		main.find('.selected-option div').empty().append(icon, phone_code);

		main.find('.selected-option input[type="hidden"][name="country_code"]').val(phone_code.text());

		main.find('.options').removeClass('active');
		main.find('.selected-option div').removeClass('active');

		main.find('.search-box').val('');
		main.find('.options').find('.hide').removeClass('hide');
	});
	$('.select-country-code .select-box .search-box').on('input',function(){
		main = $(this).parents('.select-box');
		let search_query = main.find('.search-box').val().toLowerCase(); 
